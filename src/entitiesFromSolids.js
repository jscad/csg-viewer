...
function entitiesFromSolids (baseParams, solids) {
  ...
  const entities = solids.map(function (solid) {
    ...
    if ('sides' in solid) {
      ...
      type = '3d'
      geometry = csgToGeometries(solid, {smoothLighting: baseParams.lighting.smooth, faceColor: baseParams.rendering.meshColor, normalThreshold: 0.3})//, normalThreshold: 0})
    }
